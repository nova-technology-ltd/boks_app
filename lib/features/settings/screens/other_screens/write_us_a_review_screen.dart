...
class _WriteUsAReviewScreenState extends State<WriteUsAReviewScreen> {
  ...
  Future<void> _sendReview(
    ...
    try {
      ...
      await _settingsService.writeUsASoftwareReview(
        context,
        title,
        message,
        stars,
      );
      _futureSoftwareReviewData = _settingsService.mySoftwareReview(context);
      await _getData();
      setState(() {
        isSending = false;
      });
  ...
  Future<void> _updateUserReviewData(
    ...
    try {
      ...
      await _settingsService.updateUserSoftwareReview(
  ...
  Future<void> _clearSoftwareReview(BuildContext context, String SRID) async {
    try {
      ...
      await _settingsService.clearUserSoftwareReview(context, SRID);
      setState(() {
        SRID = "";
        selectedOption = null;
        selectedStars = 0;
